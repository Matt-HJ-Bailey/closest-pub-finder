...
def main():
    ...
    assert os.path.exists(
        args.mapfile
    ), f"{args.mapfile} is not a valid path to a map file"
    assert (
    ...
    if args.colleges:
        ...
        pubs = [
            pub
            for pub in import_pubs(args.datafile)
            if pub.is_open and not pub.is_college
        ]
    logger.info("Calculating distances...")
    pubs = populate_distances(
        pubs, pubgoers, person_locations, home_locations, nx_graph
    )
    ...
    ax = plot_highlighted_paths(nx_graph, paths_highlight, ax=ax, draw_arrows=True)
    plt.show()
