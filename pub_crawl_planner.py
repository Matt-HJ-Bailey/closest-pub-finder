...
from pub_satisfier import find_all_closest_nodes, find_closest_node, load_graph, import_pubs
...
def find_shortest_crawl_path(nx_graph, pubs):
    closest_nodes = find_all_closest_nodes(pubs, nx.get_node_attributes(nx_graph, "pos"))
    ...
    for pub_i, pub_j in itertools.combinations(pubs, 2):
        ...
        logging.info(f"Shortest distance from {pub_i} to {pub_j} is {shortest_distance}")
        ip_graph.add_edge(pub_i.name, pub_j.name, length=shortest_distance)                
    
    closest_nodes["PTCL"] = find_closest_node(PTCL_LOCATION, nx.get_node_attributes(nx_graph, "pos"))
    for pub in pubs:
    ...
    path = nx.approximation.traveling_salesman_problem(ip_graph, weight="length", cycle=True)
    
    total_path = []
    for i in range(len(path) - 1):
        node_i, node_j = closest_nodes[path[i]], closest_nodes[path[i+1]]
        between_path = nx.shortest_path(nx_graph, source=node_i, target=node_j, weight="length")
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=["The Turf Tavern", "The Chequers", "The Kings Arms", "White Horse", "The Royal Oak", "The Bear Inn"],
    )
    ...
    logger.info("Creating an intra-pub graph of " + ",".join(pub.name for pub in valid_crawl_pubs))
    
    shortest_path, name_path = find_shortest_crawl_path(nx_graph, valid_crawl_pubs)
    paths_highlight = [[
    
                (sublist[node], sublist[node + 1])
                for node in range(len(sublist) - 1)
            ]
